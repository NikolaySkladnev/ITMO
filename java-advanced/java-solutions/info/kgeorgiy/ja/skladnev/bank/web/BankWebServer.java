package info.kgeorgiy.ja.skladnev.bank.web;
...
//    <td>{level}</td>
    ...
    private final Map<String, App> apps = new TreeMap<>(Map.ofEntries(
            app("Client 1", "java", "-cp", CLASSPATH, "info.kgeorgiy.ja.skladnev.bank.client.Client", "client-1"),
            app("Client 2", "java", "-cp", CLASSPATH, "info.kgeorgiy.ja.skladnev.bank.client.Client", "client-2"),
            app("Client 3", "java", "-cp", CLASSPATH, "info.kgeorgiy.ja.skladnev.bank.client.Client", "client-3"),
            app("Server 1", "java", "-cp", CLASSPATH, "info.kgeorgiy.ja.skladnev.bank.server.Server", "8881"),
            app("Server 2", "java", "-cp", CLASSPATH, "info.kgeorgiy.ja.skladnev.bank.server.Server", "8882"),
    ...
    private static String escapeHTML(final String value) {
        return value.chars()
                .mapToObj(c -> HTML_ESCAPES.getOrDefault(Optional.of(c), Character.toString(c)))
                .collect(Collectors.joining());
    }
